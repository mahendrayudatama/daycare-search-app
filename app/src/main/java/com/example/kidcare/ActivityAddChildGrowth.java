...
public class ActivityAddChildGrowth extends AppCompatActivity implements MaterialPickerOnPositiveButtonClickListener, View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        weight = findViewById(R.id.et_berat_badan);
        height = findViewById(R.id.et_tinggi_badan);
        date = findViewById(R.id.et_date_picker_growth);
        submitGrowth = findViewById(R.id.btn_submitChildGrowth);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.et_date_picker_growth:
    ...
    void postData() {
        if (weight.getText().toString().isEmpty()) {
            weight.setError("mohon isi data");
            weight.requestFocus();
            return;
        }
        if (height.getText().toString().isEmpty()){
            height.setError("mohon isi data");
            height.requestFocus();
            return;
        }
        if (date.getText().toString().isEmpty()){
            date.requestFocus();
            date.setError("mohon isi data");
            return;
                    ...
                    current_user_db.child(task.getResult().getValue().toString()).child("birthDate").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            ...
                            try {
                                ...
                                current_user_db.child(currentChild[0]).child("growth").push().setValue(modelGrowth).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        Toast.makeText(ActivityAddChildGrowth.this, "Data berhasil ditambahkan", Toast.LENGTH_SHORT).show();
                                        finish();
                                    }
                                });
